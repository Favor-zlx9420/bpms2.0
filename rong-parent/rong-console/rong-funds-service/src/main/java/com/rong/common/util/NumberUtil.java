...
public class NumberUtil {
	...
	public static BigDecimal getScale(BigDecimal oldBigdecimal,int scale){
		BigDecimal setScale = oldBigdecimal.setScale(scale,BigDecimal.ROUND_HALF_DOWN);
		return setScale;
	}
	...
	public static String respScale2MDown(Double value){
		return respScale2MDown(BigDecimal.valueOf(value));
	}
	public static String respScale2MDown(BigDecimal value){
		BigDecimal b = value.setScale(2,BigDecimal.ROUND_HALF_DOWN);
		return String.valueOf(b);
	}
	public static String respScale2MDownC(Double value){
		if(null == value){
			return "";
		}
		//万元以下
		if(value < 10000){
			return toThousandth2(value);
		}else if(value < 10000_0000){
			return toThousandth0(value/10000) + "万";
		}else{
			return toThousandth0(value/10000_0000) + "亿";
		}
	}
	 ...
	 * @return
	 */
	public static Map<String,Object> getPerMonthPrincipal0(double principal, double yearRate, int totalmonth,int monthOfYear) {
		double monthRate = yearRate / monthOfYear;
		//每个月偿还总金额
		double monthAmount = principal * monthRate * Math.pow(1 + monthRate, totalmonth)/(Math.pow(1 + monthRate, totalmonth) - 1);
		//每个月利息
		double[] perInterests = getPerMonthInterest0(principal, yearRate, totalmonth,monthOfYear);
		double[] perPrincipals = new double[perInterests.length];
		double restPrincipal = principal;
	 ...
	 * @return
	 */
	public static double[] getPerMonthInterest0(double principal, double yearRate, int totalmonth,int monthOfYear) {
		double monthRate = yearRate/monthOfYear;
		double monthInterest;
		double[] perInterests = new double[totalmonth];
		for (int i = 1; i < totalmonth + 1; i++) {
			double multiply = principal*monthRate;
			double sub  = Math.pow(1 + monthRate, totalmonth) - Math.pow(1 + monthRate, i-1);
			monthInterest = multiply * sub/(Math.pow(1 + monthRate, totalmonth) - 1);
	 ...
	 * @return
	 */
	private static long half_even_up(Double cent){
		//先乘以 10000 四舍五入
