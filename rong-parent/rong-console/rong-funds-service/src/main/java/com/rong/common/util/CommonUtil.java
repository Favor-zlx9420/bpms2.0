...
import com.rong.common.annotation.*;
...
public class CommonUtil {
	...
	public static boolean inNumbers(Integer checkNumber,Integer... numbers){
		for(Integer n:numbers){
			if(CommonUtil.isEqual(checkNumber,n)){
				return true;
			}
		}
		return false;
	...
	public static boolean isNormalSql(String sql){
		return StringUtil.isNotEmpty(sql) && sql.matches("^[a-z0-9A-Z,_.]*$");
	}
    ...
    {
    	return null != strVal && strVal.matches(Validator.PHONE_NUMBER);
    }
	...
	public static Result getResultByThrowable(Throwable e){
		...
		if(e instanceof CustomerException){
			...
			result = Result.miss(CommonEnumContainer.ResultStatus.ABNORMAL_OPERATION);
		}
		return result;
	}
	public final static void formatInsertEntity(BaseEntity be){
		be.setCreateDate(new Date());
		be.setDeltag(CommonEnumContainer.Deltag.NORMAL.getValue());
	}
		...
		for (; clazz != Object.class; clazz = clazz.getSuperclass()) {
			...
			for (Field f : field) {
				...
				if (CommonUtil.isNotNull(require) && isNull) {
					...
					return Result.miss(CommonEnumContainer.ResultStatus.PARAMETER_IS_NOT_COMPLETE,"参数[" + _name + "]必填");
				}
				...
				if (ov instanceof Integer) {
					...
					if (!isNull) {
						...
						if (value < min || value > max) {
							return Result.miss(CommonEnumContainer.ResultStatus.THE_PARAMETERS_DO_NOT_MEET_THE_REQUIREMENTS,"参数[" + f.getName() + "]范围的范围应为：{" + min + "，" + max + "}");
						}
					}
					...
					if (!isNull) {
						...
						if (!((String) ov).matches(regex.regexStr())) {
							return Result.miss(CommonEnumContainer.ResultStatus.THE_PARAMETERS_DO_NOT_MEET_THE_REQUIREMENTS,String.format(regex.errorContent(),f.getName(),regex.regexStr()));
						}
					}
					...
					if(className.indexOf("com.rong") > -1) {
	 ...
	 * @return 1
	 */
	public static String createOrderNo(String OrderTypeStr, Long serviceId, Long userId, boolean isMilSeconds) {
		...
		//4位会员
		setCharOfString(oc, String.valueOf(userId), 7, 4);
		//11位时间戳
		if (isMilSeconds) {
	...
	public static String respBigDecimalWithPer(BigDecimal bigDecimal){
		return NumberUtil.getScale2M(bigDecimal) + "%";
	}
}
