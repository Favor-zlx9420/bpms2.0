...
/***************************************
*跨域上传文件 设置：在KUploadButton(options) 方法下面的extend后面submit 提交设置为ajaxsubmit，
*另在初始化要加个post_origin 参数：只有为true的时候才ajaxsubmit
*另外，还需要在image、flash、media等文件上设置post_origin属性，否则之后提交查询找不到
***************************************/
...
var _options = {
	...
	items : [
		...
		'flash', 'media', 'insertfile', 'table', 'hr', 'code', 'pagebreak', 'link', 'unlink','about'
	],
 ...
 ['#FF99CC', '#FFCC99', '#FFFF99', '#808080', '#999999', '#C0C0C0', '#CCCCCC', '#FFFFFF']
	...
	layout : '<div class="container"><div class="toolbar"></div><div class="edit"></div><div class="statusbar" style="cursor:default;height:3px;"></div></div>'
};
...
//			row = table.insertRow(colors.length + 1);
//			cell = row.insertCell(0);
//			cell.colSpan=3;
//			self._addAttr(cell,'#999999', 'ke-colorpicker-cell');
	},
...
_extend(KUploadButton, {
	...
	submit : function() {//改用JQuery.ajaxsubmit了，因为要跨域传输
		var self = this,
			iframe = self.iframe;
		...
		if(self.options.post_origin){
			...
			top.km = top.lBox.wait({content:'正在上传，请稍后',zIndex:1000000});
			$(self.form[0]).ajaxSubmit(p_options);
		}else{
			self.form[0].submit();
		}
		return self;
