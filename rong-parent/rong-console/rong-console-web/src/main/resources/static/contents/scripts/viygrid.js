...
function ViyGrid(){
    ...
    if (typeof ViyGrid.init == "undefined"){
        ...
        ViyGrid.prototype.bindEvts = function(){
            ...
            $(p.box).on("mouseover",".viy-ui-thead-box th,.viy-ui-tbody-box tr",function(ee){
                ...
                if($this.is(".list-ckb")){
                    ...
                    if(_page == p.pageInfo.pageIndex){
                        return;
                    }
                    p.pageInfo.pageIndex = _page;
                    if(p.pageInfo.pageIndex > p.pageInfo.maxpage){
                        p.pageInfo.pageIndex = p.pageInfo.maxpage;
                    }
                    //gotoPage
                    _this.reload(p);
        ...
        ViyGrid.prototype.reload = function(_p){
            ...
            var m = lBox.wait({content:"正在加载，请稍后...",isLock:false});
            p.queryParams["pageInfo." + p.insteadName["pageIndex"]] = p.pageInfo.pageIndex;
            ...
            $.post(p.action,p.queryParams,function(_data){
                if(checkResultIsOk(_data)) {
                    ...
                    var __d = data["pageInfo"];
                    if (null == __d) {
                        __d = data;
                    }
                    p.pageInfo.pageIndex = __d[p.insteadName["pageIndex"]];
                    p.pageInfo.pageSize = __d[p.insteadName["pageSize"]];
                    p.pageInfo.recordCount = __d[p.insteadName["recordCount"]];
                    _this.bindTbody();
                    _this.bindPager();
...
ViyGrid.loadTheadItem = function(p,theads,pre,suffix){
    ...
    for(var i = 0,j=theads.length;i<j;++i){
        ...
        var w = th["rowWidth"];
        if(th["sortable"] == "1"){
            if(gf(th["tableAlias"],th["fieldName"]) == p.pageInfo.sortField){
                sb.append(" data-sort-dis='").append(p.pageInfo.sortDistanct).append("'");
            }
            sb.append(" data-sort-field='");
            sb.append(gf(th["tableAlias"],th["fieldName"])).append("' ");
            sb.append(" ><div style='width:").append(w).append("px'><span>").append(th["tableHeader"]).append("</span><i class='sortable asc icon icon-sort-up'></i><i class='sortable desc icon icon-sort-down'></i></div></th>");
        }else{
            sb.append(" ><div style='width:").append(w).append("px'>").append(th["tableHeader"]).append("</div></th>");
        }
    }
    sb.append(suffix);
    sb.append("</tr></thead>");
...
ViyGrid.getListDivValue = function(tr,v,i,td,fn,treeFields){
    ...
    if(null == tpl || typeof tpl == "string"){
        if(LE.isEmpty(tpl)){
            ...
            //fc
            sb.append(window[tpl](tr,v,i,td["fieldName"]));//
        }
    }else if(typeof tpl == "function"){
        sb.append(tpl(tr,v,i,td["fieldName"]));//
    }
    return sb;
};
...
ViyGrid.page = function(pageInfo){
    ...
    pageInfo.pageIndex = pageInfo.pageIndex > maxpage ? maxpage : pageInfo.pageIndex;
    ...
    if(pageInfo.pageIndex<=1){
        ...
        strB.append(ViyGrid.getLinkDiv(pageInfo.pageIndex - 1, "上一页", "lastPage", "上一页"));//上一页
    }
    if(pageInfo["renderType"] != 1) {
        if (maxpage > 5) {
            //如果当前页>3;
            var afTen = pageInfo.pageIndex + 10 < maxpage ? (pageInfo.pageIndex + 10) : maxpage;
            if (pageInfo.pageIndex > 3) {
                var preTen = pageInfo.pageIndex - 10 > 0 ? (pageInfo.pageIndex - 10) : 1;
                strB.append(ViyGrid.getLinkDiv(preTen, "前十页", "", "···"));
                if (pageInfo.pageIndex <= maxpage - 3) {//若为<倒数第三页
                    strB.append(ViyGrid.getLinkDiv(pageInfo.pageIndex - 2, "第" + (pageInfo.pageIndex - 2) + "页", "", (pageInfo.pageIndex - 2) + ""));//当前页-2
                    strB.append(ViyGrid.getLinkDiv(pageInfo.pageIndex - 1, "第" + (pageInfo.pageIndex - 1) + "页", "", (pageInfo.pageIndex - 1) + ""));//当前页-1
                    //当前页
                    strB.append("<a data-page='").append(pageInfo.pageIndex).append("' class='currentAhur'>");
                    strB.append(pageInfo.pageIndex);
                    strB.append("</a>");
                    ...
                    //显示前面的
                    for (var i = 5 - (maxpage - pageInfo.pageIndex) - 1; i > 0; i--) {
                        strB.append(ViyGrid.getLinkDiv((pageInfo.pageIndex - i), "", "", (pageInfo.pageIndex - i) + ""));
                    }
                    //显示后面的；!
                    for (var i = 0; i <= maxpage - pageInfo.pageIndex; i++) {
                        if (pageInfo.pageIndex != pageInfo.pageIndex + i) {
                            strB.append(ViyGrid.getLinkDiv((pageInfo.pageIndex + i), "", "", (pageInfo.pageIndex + i) + ""));
                        }
                        else {
                            strB.append("<a data-page='").append(pageInfo.pageIndex).append("' class='currentAhur'>").append(pageInfo.pageIndex).append("</a>");
                        }
                    }
                }
            }
            else {//当前页小于3
                strB.append(ViyGrid.getLinkList(5, pageInfo.pageIndex));
                strB.append(ViyGrid.getLinkDiv(afTen, "后十页", "", "···"));
            }
        } else {
            strB.append(ViyGrid.getLinkList(maxpage, pageInfo.pageIndex));
        }
    }
    if (pageInfo.pageIndex != maxpage)
    {
        strB.append(ViyGrid.getLinkDiv((pageInfo.pageIndex + 1) < maxpage ? (pageInfo.pageIndex + 1) : maxpage,  "下一页","nextPage", "下一页"));//下一页
    ...
    if (maxpage>1)
    {
        strB.append("<input value='").append(pageInfo.pageIndex).append("' type='text' class='input checkIsFloat' style='height:22px;line-height:22px;width:40px;margin:0px 3px;' /><a class='btn-page-post'>GO</a>");
    }
    strB.append("<span>");
    strB.append(pageInfo.pageIndex);
...
ViyGrid.pageEvent = function(box,pageInfo,pageFunc,refreshFunc){
    $(box).on("click","a:not(.disabledlink),.btn-refresh",function(){
        ...
        if(!$this.is(".btn-refresh")){
            ...
            if(_page == pageInfo.pageIndex){
                //return;
            }
            pageInfo.pageIndex = _page;
            if(pageInfo.pageIndex > pageInfo.maxpage){
                pageInfo.pageIndex = pageInfo.maxpage;
            }
            //gotoPage
            pageFunc(pageInfo);
        }else{
